...
class stock {
  ...
  // Financials.
  financials = async (period: 'annual' | 'quarter' = 'quarter'): Promise<Financials> => await this.handle(this.path('/financials'), {period})
  ...
  // Quote.
  quote = async (displayPercent?: boolean): Promise<Quote> => await this.handle('/quote', {displayPercent})
